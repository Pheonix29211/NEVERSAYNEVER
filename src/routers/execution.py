...
def _default_key_path() -> Optional[str]:
    ...
    # common fallbacks
    for p in (os.path.join(Cfg.DATA_DIR, "my_wallet_key.json"),
              os.path.join(Cfg.DATA_DIR, "phantom_key.json")):
        if os.path.exists(p):
            return p
    return None
...
def _decode_json_array(s: str) -> Optional[bytes]:
    """Parse JSON array like [1,2,...] -> bytes; expects length 32 or 64."""
    try:
        arr = json.loads(s)
...
# ---------- Load solders keypair ----------
def _load_keypair():
    """
    Return a solders.Keypair from 64-byte secret (preferred) or 32-byte seed (if supported).
    """
    try:
...
def _load_keypairs() -> Tuple[Optional[object], Optional[object]]:
    """
    For API compatibility with older code that expects two return values.
    Returns: (kp_solders, None) — we only use solders keypair in this build.
    """
    return _load_keypair(), None
...
# ---------- Execution Engine (solders sign + solana send) ----------
class ExecutionEngine:
    ...
    async def execute_buy(self, mint: str, usd_amount: float, route_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Build unsigned swap tx (bytes)
        async with aiohttp.ClientSession() as session:
            raw = await self._build_swap_tx(session, route_info, str(kp_solders.pubkey()))
            if not raw:
                return {"ok": False, "reason": "swap_build"}

        # Sign with solders (no tx.sign in 0.26): create a new VersionedTransaction(message, [sig])
        try:
